...
/**
 * Script: Erstellt einen automatisch generierten 10-stelligen Passcode
 * über den /v3/keyboardPwd/get Endpunkt (Random Passcode)
 * 
 * WICHTIG: Dieser Endpunkt funktioniert OHNE Gateway und OHNE App-Sync!
 * Die API generiert automatisch einen 10-stelligen Passcode.
 */
async function createAutoPasscodeNoSync() {
  try {
    console.log('🚀 Erstelle automatisch generierten 10-stelligen Passcode (ohne Gateway, ohne App-Sync)\n');
    ...
    endDate.setDate(endDate.getDate() + 2); // +2 Tage (anderer Zeitraum für neuen Code)
    ...
    payload.append('keyboardPwdName', `Auto Passcode Test ${Date.now()}`);
    ...
    if (generatedPasscode) {
      ...
      console.log(`\n📝 WICHTIG: Dieser Code wurde über /v3/keyboardPwd/get erstellt`);
      console.log(`   - Kein Gateway erforderlich`);
      console.log(`   - Keine App-Synchronisation erforderlich`);
      console.log(`   - Sollte sofort an der Tür funktionieren`);
