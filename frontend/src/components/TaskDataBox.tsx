...
const TaskDataBox: React.FC<TaskDataBoxProps> = ({ task }) => {
  ...
  const handleCopyData = async () => {
    try {
      ...
      showMessage(t('lifecycle.dataCopied') || 'Daten in Zwischenablage kopiert', 'success');
    } catch (error) {
      console.error('Fehler beim Kopieren:', error);
      showMessage(t('lifecycle.copyError') || 'Fehler beim Kopieren', 'error');
    }
  };
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-300 dark:border-gray-700 p-6 mb-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold flex items-center dark:text-white">
          ...
          {t('lifecycle.autoGeneratedData') || 'Automatisch generierte Daten'}
        </h3>
        <button
          onClick={handleCopyData}
          className="text-blue-600 dark:text-blue-400 hover:underline text-sm"
        >
          {t('lifecycle.copyData') || 'Daten kopieren'}
        </button>
      </div>
      <div className="space-y-2">
        {Object.keys(autoGeneratedData).length > 0 ? (
          ...
          <div className="text-gray-500 dark:text-gray-400 text-sm">
            {t('lifecycle.noDataAvailable') || 'Keine automatisch generierten Daten verfügbar'}
          </div>
        )}
      </div>
    </div>
