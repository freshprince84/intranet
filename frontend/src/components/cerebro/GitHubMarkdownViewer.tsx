...
const GitHubMarkdownViewer: React.FC<GitHubMarkdownViewerProps> = ({ 
  ...
  const components = {
    ...
    h1: (props: any) => <h1 className="text-2xl font-bold mb-4 border-b border-gray-200 pb-2" {...props} />,
    h2: (props: any) => <h2 className="text-xl font-bold mt-6 mb-4 border-b border-gray-200 pb-2" {...props} />,
    h3: (props: any) => <h3 className="text-lg font-bold mt-5 mb-3" {...props} />,
    ...
    // Tabellen
    table: (props: any) => <table className="border-collapse border border-gray-300 my-4" {...props} />,
  ...
  return (
    <div className="github-markdown-content">
      <div className="bg-gray-100 p-2 mb-4 rounded text-sm flex justify-between items-center">
        <span>
          Datei: <code className="bg-white px-1 py-0.5 rounded">{path}</code>
        </span>
        <a 
          href={`https://github.com/${owner}/${repo}/blob/${branch}/${path}`}
          target="_blank"
