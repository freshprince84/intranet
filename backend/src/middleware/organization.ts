...
export const getDataIsolationFilter = (req: Request, entity: string): any => {
  ...
  switch (entity) {
    case 'task':
      ...
      logger.warn('[getDataIsolationFilter] Task: permissionContext nicht vorhanden, verwende sicheren Fallback (nur eigene Tasks)');
      ...
      logger.log('[getDataIsolationFilter] Task fallback filter (nur eigene Tasks):', JSON.stringify(taskFilter, null, 2));
