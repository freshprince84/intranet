...
export interface PermissionEntity {
  ...
  description: string;
  parent?: string; // Für hierarchische Beziehungen (z.B. Tab gehört zu Page)
  ownershipFields?: string[]; // DB-Felder für "eigene" Daten (z.B. ['requesterId', 'responsibleId'])
}
...
export interface RolePermissionMap {
  [entityKey: string]: AccessLevel; // entityKey = `${entityType}_${entity}`
}

/**
 * Admin-Rolle: Voller Zugriff auf alles
 */
export const ADMIN_PERMISSIONS: RolePermissionMap = (() => {
 ...
 */
export const USER_PERMISSIONS: RolePermissionMap = {
  // Pages
 ...
 */
export const HAMBURGER_PERMISSIONS: RolePermissionMap = {
...
ALL_ENTITIES.forEach(e => {
  ...
  if (!(key in USER_PERMISSIONS)) {
    USER_PERMISSIONS[key] = 'none';
  }
  if (!(key in HAMBURGER_PERMISSIONS)) {
    HAMBURGER_PERMISSIONS[key] = 'none';
  }
});
