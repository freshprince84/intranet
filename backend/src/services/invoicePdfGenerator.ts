...
function generateSwissQRCodeData(invoice: ConsultationInvoice, settings: InvoiceSettings): string {
  const data = [
    // Header
    'SPC',                           // QRType
    '0200',                          // Version
...
export async function generateInvoicePDF(invoice: ConsultationInvoice, settings: InvoiceSettings): Promise<Buffer> {
...
function generateInvoiceContent(doc: PDFKit.PDFDocument, invoice: ConsultationInvoice, settings: InvoiceSettings, qrCodeBuffer: Buffer) {
