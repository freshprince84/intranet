...
export class TTLockService {
  ...
  async createTemporaryPasscode(
    ...
    try {
      ...
      // Für period Passcodes (keyboardPwdType: 3) kann Passcode 4-10 Ziffern lang sein
      if (!useAutoGenerated) {
        // Personalisierter 4-stelliger Passcode (erfordert Bluetooth-Synchronisation über TTLock App)
        // ABER: Für permanente Passcodes muss 6-9 Ziffern sein, daher verwende 6-stellig
        generatedPasscode = Math.floor(100000 + Math.random() * 900000).toString(); // 6-stellig
        console.log('[TTLock] Verwende benutzerdefinierten 6-stelligen Passcode (permanent, erfordert Synchronisation)');
      } else {
        // Automatisch generierter 6-9-stelliger Passcode für permanente Codes
        // Verwende 8-stellig für gute Balance zwischen Sicherheit und Länge
        generatedPasscode = Math.floor(10000000 + Math.random() * 90000000).toString(); // 8-stellig
        console.log('[TTLock] Verwende automatisch generierten 8-stelligen Passcode (permanent, ohne Synchronisation)');
      }
