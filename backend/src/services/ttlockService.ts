...
export class TTLockService {
  ...
  async createTemporaryPasscode(
    ...
    try {
      ...
      // WICHTIG: 9-stellige Passcodes funktionieren ohne App-Sync (herausgefunden und dokumentiert)
      if (!useAutoGenerated) {
        // Personalisierter Passcode (erfordert Bluetooth-Synchronisation über TTLock App)
        // Für permanente Passcodes muss 6-9 Ziffern sein, verwende 9-stellig für keine Sync
        generatedPasscode = Math.floor(100000000 + Math.random() * 900000000).toString(); // 9-stellig
        console.log('[TTLock] Verwende benutzerdefinierten 9-stelligen Passcode (permanent, ohne Synchronisation)');
      } else {
        // Automatisch generierter 9-stelliger Passcode für permanente Codes
        // 9-stellig funktioniert ohne App-Sync (herausgefunden und dokumentiert)
        generatedPasscode = Math.floor(100000000 + Math.random() * 900000000).toString(); // 9-stellig
        console.log('[TTLock] Verwende automatisch generierten 9-stelligen Passcode (permanent, ohne Synchronisation)');
      }
