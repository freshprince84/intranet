...
export class TTLockService {
  ...
  async createTemporaryPasscode(
    ...
    try {
      ...
      // Für permanente Passcodes (keyboardPwdType: 2) muss Passcode 6-9 Ziffern lang sein
      // WICHTIG: GENAU 9-stellige Passcodes funktionieren ohne App-Sync (getestet und dokumentiert)
      // MUSS GENAU 9 Ziffern sein - nicht 8, nicht 10!
      // Generiere IMMER genau 9-stellig: 100000000 bis 999999999
      const min9Digit = 100000000; // Kleinste 9-stellige Zahl
      const max9Digit = 999999999; // Größte 9-stellige Zahl
      generatedPasscode = Math.floor(min9Digit + Math.random() * (max9Digit - min9Digit + 1)).toString();
      
      // VALIDIERUNG: Stelle sicher, dass es GENAU 9 Ziffern sind
      if (generatedPasscode.length !== 9) {
        console.error(`[TTLock] FEHLER: Passcode hat falsche Länge: ${generatedPasscode.length} statt 9! Passcode: ${generatedPasscode}`);
        // Fallback: Erzwinge 9-stellig
        generatedPasscode = Math.floor(100000000 + Math.random() * 899999999).toString().padStart(9, '0');
        if (generatedPasscode.length !== 9) {
          throw new Error(`Passcode-Generierung fehlgeschlagen: Länge ${generatedPasscode.length} statt 9`);
        }
      }
      
      console.log(`[TTLock] Generierter Passcode: ${generatedPasscode} (Länge: ${generatedPasscode.length}, permanent, ohne Synchronisation)`);
