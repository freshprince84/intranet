...
export class TTLockService {
  ...
  async createTemporaryPasscode(
    ...
    try {
      ...
      // WICHTIG: Für automatische Generierung ohne App-Sync: keyboardPwd WEGLASSEN!
      // Wenn keyboardPwd weggelassen wird, generiert die API automatisch einen 10-stelligen Code
      // Dieser funktioniert ohne App-Synchronisation
      // Nur für custom (4-stellig) setzen wir keyboardPwd
      if (!useAutoGenerated && generatedPasscode) {
        payload.append('keyboardPwd', generatedPasscode);
      }
      // keyboardPwd NICHT setzen für auto - API generiert automatisch 10-stellig
      ...
      // Für automatisch generierte Codes ohne App-Sync: addType: 2 (Gateway/WiFi)
      payload.append('addType', '2'); // 2 = via gateway/WiFi (funktioniert ohne App-Sync)
      ...
      // Wenn keyboardPwd weggelassen wurde, generiert die API den Passcode automatisch (10-stellig)
      if (responseData.errcode === 0) {
        // API gibt möglicherweise den generierten Passcode zurück
        if (responseData.data?.keyboardPwd) {
          const apiGeneratedPasscode = responseData.data.keyboardPwd.toString();
          console.log('[TTLock] ✅ Passcode erfolgreich erstellt! API-generierter Passcode:', apiGeneratedPasscode);
          return apiGeneratedPasscode;
        } else if (responseData.data?.passcode) {
          const apiGeneratedPasscode = responseData.data.passcode.toString();
          console.log('[TTLock] ✅ Passcode erfolgreich erstellt! API-generierter Passcode:', apiGeneratedPasscode);
          return apiGeneratedPasscode;
        } else if (responseData.keyboardPwd) {
          const apiGeneratedPasscode = responseData.keyboardPwd.toString();
          console.log('[TTLock] ✅ Passcode erfolgreich erstellt! API-generierter Passcode:', apiGeneratedPasscode);
          return apiGeneratedPasscode;
        }
        // Falls kein Passcode zurückgegeben wird, verwende unseren generierten (nur für custom)
        if (!useAutoGenerated && generatedPasscode) {
          return generatedPasscode;
        }
        throw new Error('API hat keinen Passcode zurückgegeben (erwartet für auto)');
        ...
        if (responseData.keyboardPwd) {
          const apiGeneratedPasscode = responseData.keyboardPwd.toString();
          console.log('[TTLock] API-generierter Passcode:', apiGeneratedPasscode);
          return apiGeneratedPasscode;
        } else if (responseData.passcode) {
          const apiGeneratedPasscode = responseData.passcode.toString();
          console.log('[TTLock] API-generierter Passcode:', apiGeneratedPasscode);
          return apiGeneratedPasscode;
        }
        // Für auto: API muss Passcode zurückgeben
        if (useAutoGenerated) {
          throw new Error('API hat keinen Passcode zurückgegeben (erwartet für auto)');
        }
