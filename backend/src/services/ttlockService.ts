...
export class TTLockService {
  ...
  async createTemporaryPasscode(
    ...
    try {
      ...
      // TEST: Für automatisch generierte Codes NICHT keyboardPwd setzen!
      // Die API soll den Code automatisch generieren (funktioniert ohne Sync)
      // Nur für custom Codes keyboardPwd setzen
      if (!useAutoGenerated && generatedPasscode) {
        payload.append('keyboardPwd', generatedPasscode);
      }
      // Für auto: keyboardPwd NICHT setzen - API generiert automatisch
      ...
      if (responseData.errcode === 0) {
        ...
        // Falls kein Passcode zurückgegeben wird, prüfe ob wir einen generiert haben
        // Für auto-generierte Codes sollte die API den Code zurückgeben
        if (generatedPasscode && !useAutoGenerated) {
          console.log('[TTLock] ✅ Passcode erfolgreich erstellt! Verwende generierten Passcode:', generatedPasscode);
          return generatedPasscode;
        }
        // Für auto-generierte Codes: API muss den Code zurückgeben
