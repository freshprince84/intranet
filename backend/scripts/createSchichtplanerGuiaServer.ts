 ...
 * AUSFÜHRUNG AUF SERVER: npx ts-node scripts/createSchichtplanerGuiaServer.ts
 */
async function createSchichtplanerGuia() {
  try {
    ...
    // Lese die Guía-Datei
    // Auf Server: /var/www/intranet/backend/scripts/ -> ../../ -> /var/www/intranet/
    const repoRoot = path.resolve(__dirname, '../../');
    let guiaPath = path.join(repoRoot, 'docs/implementation_plans/SCHICHTPLANER_GUIA_USUARIO_ES.md');
    
    // Fallback: Wenn nicht gefunden, versuche direkt im intranet-Verzeichnis
    if (!fs.existsSync(guiaPath)) {
      ...
      if (fs.existsSync(altPath)) {
        ...
        throw new Error(`Datei nicht gefunden. Gesucht in:\n- ${guiaPath}\n- ${altPath}\n\nBitte 'git pull' ausführen oder Datei manuell kopieren.`);
      }
    }
