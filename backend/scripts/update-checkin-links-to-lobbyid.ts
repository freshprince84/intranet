...
import { generateLobbyPmsCheckInLink } from '../src/utils/checkInLinkUtils';
  ...
  try {
    ...
    for (const reservation of reservations) {
      ...
      try {
        ...
        const newCheckInLink = generateLobbyPmsCheckInLink({
          id: reservation.id,
          lobbyReservationId: reservation.lobbyReservationId,
        ...
        if (reservation.sentMessage) {
          ...
          const oldLinkPattern = new RegExp(
            `https://app\\.lobbypms\\.com/checkinonline/confirmar\\?codigo=${reservation.id}[^\\s]*`,
            'g'
          );
