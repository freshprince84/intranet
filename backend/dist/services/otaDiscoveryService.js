...
exports.OTADiscoveryService = void 0;
...
class OTADiscoveryService {
     ...
     */
    static discoverListings(city, country, roomType, platform) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.logger.warn(`[OTADiscoveryService] 🔍 Starte Discovery für ${platform}, Stadt: ${city}, Land: ${country || 'N/A'}, Zimmertyp: ${roomType}`);
            switch (platform.toLowerCase()) {
                case 'booking.com':
                    return yield this.scrapeBookingComSearch(city, country, roomType);
                case 'hostelworld.com':
                case 'hostelworld':
                    return yield this.scrapeHostelworldSearch(city, country, roomType);
                default:
     ...
     */
    static scrapeBookingComSearch(city, country, roomType) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.logger.warn(`[OTADiscoveryService] 🔍 Scrape Booking.com für ${city}, ${country || 'N/A'}, ${roomType} (mit Puppeteer)`);
            const listings = [];
            let browser = null;
            let page = null;
            try {
                ...
                logger_1.logger.warn(`[OTADiscoveryService] 📡 Starte Browser für Booking.com: ${url}`);
                ...
                try {
                    ...
                    logger_1.logger.error(`[OTADiscoveryService] ⚠️ Puppeteer kann nicht gestartet werden: ${puppeteerError.message}`);
                    logger_1.logger.warn(`[OTADiscoveryService] 🔄 Fallback auf axios (ohne Browser-Automation)`);
                ...
                logger_1.logger.warn(`[OTADiscoveryService] 🌐 Navigiere zu Booking.com...`);
                yield page.goto(url, {
                ...
                if (pageContent.includes('challenge-container') || pageContent.includes('AwsWafIntegration') || pageContent.includes('verify that you\'re not a robot')) {
                    logger_1.logger.error(`[OTADiscoveryService] ⚠️ Booking.com Bot-Schutz aktiv!`);
                    logger_1.logger.error(`[OTADiscoveryService] ⚠️ Response-Länge: ${pageContent.length} Zeichen`);
                    return [];
                }
                ...
                for (const selector of selectors) {
                    try {
                        ...
                        listingsFound = true;
                        logger_1.logger.warn(`[OTADiscoveryService] ✅ Selektor "${selector}" gefunden`);
                        break;
                    }
                    catch (e) {
                ...
                if (!listingsFound) {
                    logger_1.logger.warn(`[OTADiscoveryService] ⚠️ Keine Hotel-Listings gefunden mit erwarteten Selektoren`);
                }
                // Extrahiere HTML und parse mit Cheerio
                const html = yield page.content();
                const $ = cheerio.load(html);
                logger_1.logger.warn(`[OTADiscoveryService] 📄 HTML-Response erhalten: ${html.length} Zeichen`);
                ...
                for (const selector of testSelectors) {
                    ...
                    if (count > 0) {
                        logger_1.logger.warn(`[OTADiscoveryService] 🔍 Selektor "${selector}": ${count} Elemente gefunden`);
                    }
                }
                // Extrahiere Listings
                const pageListings = this.extractBookingComListings($, roomType, city, country);
                listings.push(...pageListings);
                logger_1.logger.warn(`[OTADiscoveryService] ✅ Booking.com Discovery abgeschlossen: ${listings.length} Listings gefunden`);
            }
            catch (error) {
                ...
                if (error.message === 'PUPPETEER_UNAVAILABLE' || error.message.includes('Failed to launch the browser process')) {
                    logger_1.logger.warn(`[OTADiscoveryService] 🔄 Puppeteer nicht verfügbar, verwende Fallback auf axios`);
                    return yield this.scrapeBookingComSearchFallback(city, country, roomType);
                }
                logger_1.logger.error(`[OTADiscoveryService] ❌ Fehler beim Scraping von Booking.com:`, error.message);
                logger_1.logger.error(`[OTADiscoveryService] Error Stack:`, error instanceof Error ? error.stack : 'Kein Stack verfügbar');
            }
            finally {
                // Schließe Browser
                if (page) {
     ...
     */
    static scrapeBookingComSearchFallback(city, country, roomType) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.logger.warn(`[OTADiscoveryService] 🔍 Scrape Booking.com (Fallback axios) für ${city}, ${country || 'N/A'}, ${roomType}`);
            const listings = [];
            try {
                ...
                const encodedSearch = encodeURIComponent(searchString);
                const url = `https://www.booking.com/searchresults.de.html?ss=${encodedSearch}&dest_type=city&checkin=${checkInDate}&checkout=${checkOutDate}&group_adults=1&no_rooms=1&group_children=0&nflt=ht_id%3D203&offset=0`;
                logger_1.logger.warn(`[OTADiscoveryService] 📡 Request zu Booking.com (Fallback): ${url}`);
                const headers = {
                ...
                if (responseText.includes('challenge-container') || responseText.includes('AwsWafIntegration')) {
                    logger_1.logger.error(`[OTADiscoveryService] ⚠️ Booking.com Bot-Schutz aktiv (Fallback)!`);
                    return [];
                }
                const $ = cheerio.load(response.data);
                const pageListings = this.extractBookingComListings($, roomType, city, country);
                listings.push(...pageListings);
                logger_1.logger.warn(`[OTADiscoveryService] ✅ Booking.com Fallback abgeschlossen: ${listings.length} Listings gefunden`);
            }
            catch (error) {
                logger_1.logger.error(`[OTADiscoveryService] ❌ Fehler beim Fallback-Scraping von Booking.com:`, error.message);
            }
            return listings;
        });
    }
     ...
     * @returns Gefundene Listings
     */
    static scrapeHostelworldSearch(city, country, roomType) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.logger.warn(`[OTADiscoveryService] 🔍 Scrape Hostelworld für ${city}, ${country || 'N/A'}, ${roomType}`);
            const listings = [];
            let browser = null;
            let page = null;
            try {
                ...
                logger_1.logger.warn(`[OTADiscoveryService] 📡 Starte Browser für Hostelworld: ${url}`);
                ...
                try {
                    ...
                    logger_1.logger.warn(`[OTADiscoveryService] 🌐 Navigiere zu Hostelworld...`);
                    yield page.goto(url, {
                        waitUntil: 'networkidle2',
                        timeout: 60000
                    });
                    ...
                    try {
                        ...
                        logger_1.logger.warn(`[OTADiscoveryService] ✅ Loading-Container verschwunden und Listings geladen`);
                    }
                    catch (e) {
                        logger_1.logger.warn(`[OTADiscoveryService] ⚠️ Timeout beim Warten auf Loading-Container/Listings, warte zusätzlich...`);
                    ...
                    for (const selector of selectors) {
                        try {
                            yield page.waitForSelector(selector, { timeout: 5000 });
                            const count = yield page.$$eval(selector, elements => elements.length);
                            if (count > 0) {
                                listingsFound = true;
                                logger_1.logger.warn(`[OTADiscoveryService] ✅ Hostelworld Selektor "${selector}" gefunden: ${count} Links`);
                                break;
                            }
                        }
                        catch (e) {
                    ...
                    if (!listingsFound) {
                        ...
                        logger_1.logger.warn(`[OTADiscoveryService] ⚠️ Keine Listings-Selektoren gefunden, warte 5 Sekunden auf JavaScript...`);
                        yield new Promise(resolve => setTimeout(resolve, 5000));
                        // Prüfe nochmal, ob jetzt Links vorhanden sind
                        const linkCount = yield page.$$eval('a[href*="/hostels/"]', elements => elements.length);
                        logger_1.logger.warn(`[OTADiscoveryService] 🔍 Nach Wartezeit: ${linkCount} Hostel-Links gefunden`);
                    }
                    ...
                    listings.push(...pageListings);
                    logger_1.logger.warn(`[OTADiscoveryService] ✅ Hostelworld Discovery abgeschlossen: ${listings.length} Listings gefunden`);
                }
                catch (puppeteerError) {
                    logger_1.logger.error(`[OTADiscoveryService] ⚠️ Puppeteer kann nicht gestartet werden: ${puppeteerError.message}`);
                    logger_1.logger.warn(`[OTADiscoveryService] 🔄 Fallback auf axios (ohne JavaScript-Rendering)`);
                    // Fallback auf axios
                    return yield this.scrapeHostelworldSearchFallback(city, country, roomType);
                }
            }
            catch (error) {
                logger_1.logger.error(`[OTADiscoveryService] ❌ Fehler beim Scraping von Hostelworld:`, error.message);
                // Versuche Fallback
                return yield this.scrapeHostelworldSearchFallback(city, country, roomType);
            }
            finally {
     ...
     */
    static scrapeHostelworldSearchFallback(city, country, roomType) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.logger.warn(`[OTADiscoveryService] 🔍 Scrape Hostelworld (Fallback axios) für ${city}, ${country || 'N/A'}, ${roomType}`);
            const listings = [];
            try {
                const searchString = country ? `${city}, ${country}` : city;
                const today = new Date();
                ...
                const encodedSearch = encodeURIComponent(searchString);
                const url = `https://www.hostelworld.com/search?search_keywords=${encodedSearch}&date_from=${fromDate}&date_to=${toDate}&number_of_guests=1`;
                logger_1.logger.warn(`[OTADiscoveryService] 📡 Request zu Hostelworld (Fallback): ${url}`);
                const headers = {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8',
                    'Accept-Language': 'en-US,en;q=0.9,de;q=0.8',
                    ...
                    if (scriptContent.includes('properties') || scriptContent.includes('hostels') || scriptContent.includes('listings')) {
                        try {
                            ...
                            if (jsonMatch) {
                                ...
                                logger_1.logger.warn(`[OTADiscoveryService] ✅ JSON-Daten in Script-Tag gefunden`);
                ...
                const pageListings = this.extractHostelworldListings($, roomType, city, country);
                listings.push(...pageListings);
                logger_1.logger.warn(`[OTADiscoveryService] ✅ Hostelworld Fallback abgeschlossen: ${listings.length} Listings gefunden`);
            }
            catch (error) {
                logger_1.logger.error(`[OTADiscoveryService] ❌ Fehler beim Fallback-Scraping von Hostelworld:`, error.message);
            }
            return listings;
        });
    }
     ...
     */
    static extractBookingComListings($, roomType, city, country) {
        const listings = [];
        ...
        let hotelElements = null;
        let foundSelector = null;
        for (const selector of testSelectors) {
            const elements = $(selector);
            if (elements.length > 0) {
                logger_1.logger.warn(`[OTADiscoveryService] ✅ Booking.com Selektor "${selector}": ${elements.length} Elemente gefunden`);
        ...
        if (!hotelElements || hotelElements.length === 0) {
            logger_1.logger.warn(`[OTADiscoveryService] ⚠️ Booking.com: Keine Hotel-Elemente gefunden mit allen getesteten Selektoren`);
            logger_1.logger.warn(`[OTADiscoveryService] 🔍 Getestete Selektoren: ${testSelectors.join(', ')}`);
            return [];
        }
        logger_1.logger.warn(`[OTADiscoveryService] ✅ Verwende Selektor "${foundSelector}" mit ${hotelElements.length} Elementen`);
        hotelElements.each((index, element) => {
            try {
                ...
                logger_1.logger.error(`[OTADiscoveryService] Fehler beim Extrahieren eines Booking.com Listings:`, error.message);
            }
        });
        return listings;
    }
     ...
     * Berücksichtigt Dorm vs Private Unterschiede
     */
    static extractHostelworldListings($, roomType, city, country) {
        const listings = [];
        // PRIORITÄT 1: Suche direkt nach Links zu Hostels (schnellste Methode)
        const hostelLinks = $('a[href*="/hostels/"]');
        if (hostelLinks.length > 0) {
            logger_1.logger.warn(`[OTADiscoveryService] ✅ Hostelworld: ${hostelLinks.length} direkte Hostel-Links gefunden`);
            ...
            hostelLinks.each((index, element) => {
                try {
                    ...
                    logger_1.logger.warn(`[OTADiscoveryService] ✅ Hostelworld Listing ${listings.length}: ID=${propertyId}, URL=${listingUrl.substring(0, 80)}...`);
                }
                catch (error) {
                    logger_1.logger.warn(`[OTADiscoveryService] ⚠️ Fehler beim Verarbeiten von Link ${index}: ${error.message}`);
                }
            });
            if (listings.length > 0) {
                logger_1.logger.warn(`[OTADiscoveryService] ✅ Hostelworld: ${listings.length} Listings aus direkten Links extrahiert`);
                return listings;
            }
        }
        ...
        let hostelElements = null;
        let foundSelector = null;
        for (const selector of testSelectors) {
            const elements = $(selector);
            if (elements.length > 0) {
                logger_1.logger.warn(`[OTADiscoveryService] ✅ Hostelworld Selektor "${selector}": ${elements.length} Elemente gefunden`);
        ...
        if (!hostelElements || hostelElements.length === 0) {
            logger_1.logger.warn(`[OTADiscoveryService] ⚠️ Hostelworld: Keine Listings-Elemente gefunden mit allen getesteten Selektoren`);
            logger_1.logger.warn(`[OTADiscoveryService] 🔍 Getestete Selektoren: ${testSelectors.join(', ')}`);
            return [];
        }
        logger_1.logger.warn(`[OTADiscoveryService] ✅ Verwende Selektor "${foundSelector}" mit ${hostelElements.length} Elementen`);
        hostelElements.each((index, element) => {
            try {
                ...
                if (index < 2) {
                    logger_1.logger.warn(`[OTADiscoveryService] 🔍 Hostelworld Element ${index} HTML (erste 500 Zeichen): ${elementHtml.substring(0, 500)}`);
                }
                ...
                const listingUrl = relativeUrl
                    ...
                    : null;
                if (!listingUrl) {
                    logger_1.logger.warn(`[OTADiscoveryService] ⚠️ Hostelworld: Keine URL gefunden für Element ${index}`);
                    logger_1.logger.warn(`[OTADiscoveryService] 🔍 Element hat ${$el.find('a').length} Links gefunden`);
                    if ($el.find('a').length > 0) {
                        $el.find('a').each((i, a) => {
                            const href = $(a).attr('href');
                            logger_1.logger.warn(`[OTADiscoveryService] 🔍 Link ${i}: ${href}`);
                        });
                    }
                    return; // Skip wenn keine URL gefunden
                }
                ...
                if (!propertyId && listingUrl) {
                    ...
                    if (match && match[1]) {
                        ...
                        const hrefMatch = relativeUrl === null || relativeUrl === void 0 ? void 0 : relativeUrl.match(/\/hostels\/[^\/]+-(\d+)/);
                ...
                if (!propertyId) {
                    ...
                    logger_1.logger.warn(`[OTADiscoveryService] ⚠️ Hostelworld: Keine Property-ID gefunden, verwende Fallback: ${propertyId}`);
                }
                // Extrahiere Zimmernamen und -typ
                ...
                logger_1.logger.warn(`[OTADiscoveryService] 🔍 Hostelworld Listing ${index}: ID=${listingId}, URL=${listingUrl}, RoomName=${roomName}, isDorm=${isDorm}, isPrivate=${isPrivate}, roomType=${roomType}`);
                listings.push({
                    platform: 'hostelworld.com',
                    listingId,
                    listingUrl,
                ...
                logger_1.logger.error(`[OTADiscoveryService] Fehler beim Extrahieren eines Hostelworld Listings:`, error.message);
            }
        });
        return listings;
    }
}
exports.OTADiscoveryService = OTADiscoveryService;
//# sourceMappingURL=otaDiscoveryService.js.map